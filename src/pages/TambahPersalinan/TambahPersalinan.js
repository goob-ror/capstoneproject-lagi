...
const CACHE_KEY_PERSALINAN = 'formCache_tambahPersalinan';
...
const TambahPersalinan = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    () => !isEdit ? readCache(CACHE_KEY_PERSALINAN, 'activeTab', 'persalinan') : 'persalinan'
  );
  const [neonatalRiskScores, setNeonatalRiskScores] = useState([]);
  const [showRiskNotifications, setShowRiskNotifications] = useState([]);

  const [formData, setFormData] = useState(() => readCache(CACHE_KEY_PERSALINAN, 'formData', {
    tanggal_persalinan: '',
  ...
  // Baby data array
  const [bayiData, setBayiData] = useState(() => readCache(CACHE_KEY_PERSALINAN, 'bayiData', [{
    jenis_kelamin: '',
    berat_badan_lahir: '',
    panjang_badan_lahir: '',
    apgar_menit1: '',
  ...
  const [complications, setComplications] = useState(() => readCache(CACHE_KEY_PERSALINAN, 'complications', [{
    nama_komplikasi: '',
  ...
  const { clearCache } = useFormCache(CACHE_KEY_PERSALINAN, {
    formData,
    bayiData,
    complications,
    activeTab
  ...
  const bayiBBL = bayiData.map(b => b.berat_badan_lahir).join(',');
  const bayiApgar1 = bayiData.map(b => b.apgar_menit1).join(',');
  const bayiApgar5 = bayiData.map(b => b.apgar_menit5).join(',');
  const bayiAsfiksia = bayiData.map(b => b.asfiksia).join(',');
  const bayiPrematur = bayiData.map(b => b.prematur).join(',');
  const bayiGangguanNapas = bayiData.map(b => b.gangguan_napas).join(',');
  const bayiKondisi = bayiData.map(b => b.kondisi).join(',');
  const bayiKeterangan = bayiData.map(b => b.keterangan).join(',');
  ...
  useEffect(() => {
    const scores = bayiData.map((bayi, index) => {
      const score = calculateNeonatalRiskScore({
        babyData: {
          berat_badan_lahir: bayi.berat_badan_lahir,
          apgar_menit1: bayi.apgar_menit1,
          apgar_menit5: bayi.apgar_menit5,
          asfiksia: bayi.asfiksia,
          prematur: bayi.prematur,
          gangguan_napas: bayi.gangguan_napas,
          kondisi: bayi.kondisi,
          keterangan: bayi.keterangan,
          urutan_bayi: bayi.urutan_bayi
        },
        deliveryData: {
          jenis_persalinan: formData.cara_persalinan,
          komplikasi_persalinan: formData.komplikasi_ibu
        }
      });
  ...
  const TAB_ORDER = ['persalinan', 'komplikasi'];
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="main-content">
        ...
        <form className="komplikasi-form">
