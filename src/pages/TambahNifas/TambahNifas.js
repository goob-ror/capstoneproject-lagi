...
const TambahNifas = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    () => !isEdit ? readCache(CACHE_KEY_NIFAS, 'activeTab', 'kunjungan-nifas') : 'kunjungan-nifas'
  );
  ...
  const [formData, setFormData] = useState(() => readCache(CACHE_KEY_NIFAS, 'formData', {
    tanggal_kunjungan: '',
    jenis_kunjungan: 'KF1',
    pemeriksa: 'Bidan',
    tekanan_darah: '',
  ...
  // Baby data array
  const [bayiData, setBayiData] = useState(() => readCache(CACHE_KEY_NIFAS, 'bayiData', [{
    urutan_bayi: 1,
    berat_badan: '',
    panjang_badan: '',
  ...
  const [complications, setComplications] = useState(() => readCache(CACHE_KEY_NIFAS, 'complications', [{
    nama_komplikasi: '',
    kejadian: 'Saat Nifas',
    tanggal_diagnosis: '',
    rujuk_rs: false,
  ...
  const { clearCache } = useFormCache(CACHE_KEY_NIFAS, {
    formData,
    bayiData,
    complications,
    activeTab
  ...
  const TAB_ORDER = ['kunjungan-nifas', 'komplikasi'];
