...
const TambahIbu = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    () => !isEditMode ? readCache(CACHE_KEY_IBU, 'activeTab', 'data-ibu') : 'data-ibu'
  );

  const [formData, setFormData] = useState(() => readCache(CACHE_KEY_IBU, 'formData', {
    id: '',
    nik_ibu: '',
    nama_lengkap: '',
    tanggal_lahir: '',
  ...
  // Data Suami
  const [suamiData, setSuamiData] = useState(() => readCache(CACHE_KEY_IBU, 'suamiData', {
    nik_suami: '',
    nama_lengkap: '',
    tanggal_lahir: '',
    no_hp: '',
  ...
  const [riwayatPenyakit, setRiwayatPenyakit] = useState(
    () => readCache(CACHE_KEY_IBU, 'riwayatPenyakit', [])
  );
  ...
  const { clearCache } = useFormCache(CACHE_KEY_IBU, {
    formData,
    suamiData,
    riwayatPenyakit,
    activeTab
  ...
  const TAB_ORDER = ['data-ibu', 'data-suami', 'riwayat-penyakit'];
          ...
          <div className="form-section">
                    ...
                    <div className="form-row">
                      ...
                      <div className="form-group">
                        ...
                        <Select
                          id="rt"
                          name="rt"
                          value={rtOptions.map(rt => ({ value: rt, label: `RT ${rt}` })).find(opt => opt.value === formData.rt) || null}
                          onChange={(selectedOption) => {
                            ...
                            checkPosyanduAssignment(formData.kelurahan_id, newValue);
                          }}
                          options={rtOptions.map(rt => ({ value: rt, label: `RT ${rt}` }))}
                          placeholder={!formData.kelurahan_id ? 'Pilih Kelurahan dulu' : loadingRT ? 'Memuat RT...' : 'Cari RT...'}
                          isClearable
                          isSearchable
                          isDisabled={!formData.kelurahan_id || loadingRT}
                          styles={{
                            control: (base, state) => ({
                              ...base,
                              padding: '2px 6px',
                          ...
                          noOptionsMessage={() => "Tidak ada data RT"}
                        />
            ...
            <div className="form-actions">
              ...
              {isLastTab ? (
                ...
                <button type="button" className="btn-primary" onClick={handleNextTab}>
                  Selanjutnya
                </button>
              )}
            </div>
